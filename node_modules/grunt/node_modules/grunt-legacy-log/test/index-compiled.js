...
exports['Log instance'] = {
  ...
  'errorlns': function errorlns(test) {
    test.expect(2);
    var log = new Log({ grunt: this.grunt });
  ...
  'writetableln': function writetableln(test) {
    test.expect(1);
    var log = new Log();
  ...
  'writelns': function writelns(test) {
    test.expect(1);
    var log = new Log();
  ...
  'writeflags': function writeflags(test) {
    test.expect(3);
    var log = new Log();
  ...
  'hasLogged': function hasLogged(test) {
  ...
  'notverbose': function notverbose(test) {
    test.expect(15);
    var log = new Log();
    log.muted = true;
  ...
  'options.debug = true': function optionsDebugTrue(test) {
    test.expect(4);
  ...
  'options.verbose = false': function optionsVerboseFalse(test) {
    test.expect(7);
  ...
  'options.verbose = true': function optionsVerboseTrue(test) {
    test.expect(7);
  ...
  'options.debug = false': function optionsDebugFalse(test) {
    test.expect(1);
  ...
  'options.color = true': function optionsColorTrue(test) {
    test.expect(1);
  ...
  'options.color = false': function optionsColorFalse(test) {
    test.expect(1);
  ...
  'perma-bind this when passing grunt in (backcompat)': function permaBindThisWhenPassingGruntInBackcompat(test) {
    test.expect(43);
    var log = new Log({ grunt: this.grunt });
    stdoutMute();
    ['write', 'writeln', 'warn', 'error', 'ok', 'errorlns', 'oklns', 'success', 'fail', 'header', 'subhead', 'debug'].forEach(function (method) {
      var fn = log[method];
    ...
    test.doesNotThrow(function () {
      var fn = log.writetableln;fn([]);
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.writelns;fn([]);
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.writeflags;fn([]);
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.wordlist;fn([]);
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.uncolor;fn('');
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.wraptext;fn(1, '');
    }, 'Should not throw if invoked in a way where this is not log.');
    test.doesNotThrow(function () {
      var fn = log.table;fn([], '');
    }, 'Should not throw if invoked in a way where this is not log.');
...
exports['Helpers'] = {
  ...
  'wordlist': function wordlist(test) {
    test.expect(3);
    var log = new Log();
  ...
  'wraptext': function wraptext(test) {
    test.expect(9);
    var log = new Log();
  ...
  'table': function table(test) {
    test.expect(2);
    var log = new Log();

    test.strictEqual(log.table, legacyLog.table);
    test.equal(legacyLog.table([3, 1, 5, 1, 8, 1, 12, 1, 20], ['a aa aaa aaaa aaaaa', '|||||||', 'b bb bbb bbbb bbbbb', '|||||||', 'c cc ccc cccc ccccc', '|||||||', 'd dd ddd dddd ddddd', '|||||||', 'e ee eee eeee eeeee eeeeee']), 'a  |b bb |c cc ccc|d dd ddd    |e ee eee eeee eeeee \n' + 'aa |bbb  |cccc    |dddd ddddd  |eeeeee              \n' + 'aaa|bbbb |ccccc   |            |\n' + 'aaa|bbbbb|        |            |\n' + 'a  |     |        |            |\n' + 'aaa|     |        |            |\n' + 'aa |     |        |            |');
    test.done();
  } };

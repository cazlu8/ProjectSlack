...
var CHAR_DOT = 46; /* . */
var CHAR_SLASH = 47; /* / */
var CHAR_DIGIT_ZERO = 48; /* 0 */
var CHAR_DIGIT_ONE = 49; /* 1 */
var CHAR_DIGIT_NINE = 57; /* 9 */
var CHAR_COLON = 58; /* : */
var CHAR_LESS_THAN = 60; /* < */
var CHAR_GREATER_THAN = 62; /* > */
var CHAR_QUESTION = 63; /* ? */
var CHAR_COMMERCIAL_AT = 64; /* @ */
var CHAR_CAPITAL_A = 65; /* A */
var CHAR_CAPITAL_F = 70; /* F */
var CHAR_CAPITAL_L = 76; /* L */
var CHAR_CAPITAL_N = 78; /* N */
var CHAR_CAPITAL_P = 80; /* P */
var CHAR_CAPITAL_U = 85; /* U */
var CHAR_LEFT_SQUARE_BRACKET = 91; /* [ */
var CHAR_BACKSLASH = 92; /* \ */
var CHAR_RIGHT_SQUARE_BRACKET = 93; /* ] */
var CHAR_UNDERSCORE = 95; /* _ */
var CHAR_GRAVE_ACCENT = 96; /* ` */
var CHAR_SMALL_A = 97; /* a */
var CHAR_SMALL_B = 98; /* b */
var CHAR_SMALL_E = 101; /* e */
var CHAR_SMALL_F = 102; /* f */
var CHAR_SMALL_N = 110; /* n */
var CHAR_SMALL_R = 114; /* r */
var CHAR_SMALL_T = 116; /* t */
var CHAR_SMALL_U = 117; /* u */
var CHAR_SMALL_V = 118; /* v */
var CHAR_SMALL_X = 120; /* x */
...
SIMPLE_ESCAPE_SEQUENCES[CHAR_DIGIT_ZERO] = '\u0000';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_A] = '\u0007';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_B] = '\b';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_T] = '\t';
SIMPLE_ESCAPE_SEQUENCES[CHAR_TAB] = '\t';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_N] = '\n';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_V] = '\u000b';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_F] = '\f';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_R] = '\r';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SMALL_E] = '\u001b';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SPACE] = ' ';
SIMPLE_ESCAPE_SEQUENCES[CHAR_DOUBLE_QUOTE] = '"';
SIMPLE_ESCAPE_SEQUENCES[CHAR_SLASH] = '/';
SIMPLE_ESCAPE_SEQUENCES[CHAR_BACKSLASH] = '\\';
SIMPLE_ESCAPE_SEQUENCES[CHAR_CAPITAL_N] = '';
SIMPLE_ESCAPE_SEQUENCES[CHAR_UNDERSCORE] = ' ';
...
function loadAll(input, output, options) {
  ...
  function generateError(message) {
    return new YAMLException(message, new Mark(filename, input, position, line, position - lineStart));
  }
  ...
  function testDocumentSeparator() {
    ...
    if (position === lineStart && (CHAR_MINUS === character || CHAR_DOT === character) && input.charCodeAt(position + 1) === character && input.charCodeAt(position + 2) === character) {

      _position = position + 3;
      _character = input.charCodeAt(_position);

      if (_position >= length || CHAR_SPACE === _character || CHAR_TAB === _character || CHAR_LINE_FEED === _character || CHAR_CARRIAGE_RETURN === _character) {
        return true;
      }
    }
  ...
  function readPlainScalar(nodeIndent, withinFlowCollection) {
    ...
    if (CHAR_QUESTION === character || CHAR_MINUS === character) {
      ...
      if (CHAR_SPACE === following || CHAR_TAB === following || CHAR_LINE_FEED === following || CHAR_CARRIAGE_RETURN === following || withinFlowCollection && (CHAR_COMMA === following || CHAR_LEFT_SQUARE_BRACKET === following || CHAR_RIGHT_SQUARE_BRACKET === following || CHAR_LEFT_CURLY_BRACKET === following || CHAR_RIGHT_CURLY_BRACKET === following)) {
        return false;
      }
    }
    ...
    while (position < length) {
      if (CHAR_COLON === character) {
        ...
        if (CHAR_SPACE === preceding || CHAR_TAB === preceding || CHAR_LINE_FEED === preceding || CHAR_CARRIAGE_RETURN === preceding) {
          break;
        }
      } else if (position === lineStart && testDocumentSeparator() || withinFlowCollection && (CHAR_COMMA === character || CHAR_LEFT_SQUARE_BRACKET === character || CHAR_RIGHT_SQUARE_BRACKET === character || CHAR_LEFT_CURLY_BRACKET === character || CHAR_RIGHT_CURLY_BRACKET === character)) {
        break;
  ...
  function readFlowCollection(nodeIndent) {
    ...
    while (position < length) {
      ...
      if (CHAR_QUESTION === character) {
        ...
        if (CHAR_SPACE === following || CHAR_TAB === following || CHAR_LINE_FEED === following || CHAR_CARRIAGE_RETURN === following) {
  ...
  function readBlockMapping(nodeIndent) {
    ...
    while (position < length) {
      ...
      if ((CHAR_QUESTION === character || CHAR_COLON === character) && (CHAR_SPACE === following || CHAR_TAB === following || CHAR_LINE_FEED === following || CHAR_CARRIAGE_RETURN === following)) {
        ...
        if (line === _line) {
          ...
          if (CHAR_COLON === character) {
            ...
            if (CHAR_SPACE !== character && CHAR_TAB !== character && CHAR_LINE_FEED !== character && CHAR_CARRIAGE_RETURN !== character) {
  ...
  function readTagProperty() {
    ...
    if (isVerbatim) {
      ...
      while (position < length && CHAR_SPACE !== character && CHAR_TAB !== character && CHAR_LINE_FEED !== character && CHAR_CARRIAGE_RETURN !== character) {
  ...
  function readAnchorProperty() {
    ...
    while (position < length && CHAR_SPACE !== character && CHAR_TAB !== character && CHAR_LINE_FEED !== character && CHAR_CARRIAGE_RETURN !== character && CHAR_COMMA !== character && CHAR_LEFT_SQUARE_BRACKET !== character && CHAR_RIGHT_SQUARE_BRACKET !== character && CHAR_LEFT_CURLY_BRACKET !== character && CHAR_RIGHT_CURLY_BRACKET !== character) {
      character = input.charCodeAt(++position);
    }
  ...
  function composeNode(parentIndent, nodeContext, allowToSeek, allowCompact) {
    ...
    if (isIndented || CONTEXT_BLOCK_OUT === nodeContext) {
      ...
      if (isIndented) {
        if (allowBlockCollections && (readBlockSequence(blockIndent) || readBlockMapping(blockIndent)) || readFlowCollection(flowIndent)) {
          hasContent = true;
        } else {
          if (allowBlockScalars && readBlockScalar(flowIndent) || readSingleQuotedScalar(flowIndent) || readDoubleQuotedScalar(flowIndent)) {
            hasContent = true;
          } else if (readAlias()) {
            hasContent = true;
    ...
    if (null !== tag && '!' !== tag) {
      if ('?' === tag) {
        if (resolve) {
          for (typeIndex = 0, typeQuantity = implicitTypes.length; typeIndex < typeQuantity; typeIndex += 1) {
  ...
  function readDocument() {
    ...
    if (0 === lineIndent && CHAR_MINUS === character && CHAR_MINUS === input.charCodeAt(position + 1) && CHAR_MINUS === input.charCodeAt(position + 2)) {
      position += 3;
      character = input.charCodeAt(position);
      skipSeparationSpace(true, -1);
    ...
    if (validate && checkLineBreaks && PATTERN_NON_ASCII_LINE_BREAKS.test(input.slice(documentStart, position))) {

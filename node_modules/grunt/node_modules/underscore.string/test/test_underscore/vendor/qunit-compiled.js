...
(function (window) {
   ...
   */
		...
		same: function same(a, b, message) {
   ...
   */
	...
	function done() {
		...
		var banner = id("qunit-banner"),
		    tests = id("qunit-tests"),
		    html = ["Tests completed in ", +new Date() - config.started, " milliseconds.<br/>", "<span class=\"bad\">", config.stats.all - config.stats.bad, "</span> tests of <span class=\"all\">", config.stats.all, "</span> passed, ", config.stats.bad, " failed."].join("");
	...
	QUnit.jsDump = (function () {
		...
		var jsDump = {
			...
			typeOf: function typeOf(obj) {
				...
				return type != "object" && type != f ? type : !obj ? "null" : obj.exec ? "regexp" : // some browsers (FF) consider regexps functions
				obj.getHours ? "date" : obj.scrollBy ? "window" : obj.nodeName == "#document" ? "document" : obj.nodeName ? "node" : obj.item ? "nodelist" : // Safari reports nodelists as functions
				obj.callee ? "arguments" : obj.call || obj.constructor != Array && //an array would also fall on this hack
...
(function (window) {
	...
	var Test = function Test(name, testName, expected, testEnvironmentArg, async, callback) {
		this.name = name;
   ...
   */
		...
		log: function log(result, message) {},
		testStart: function testStart(name, testEnvironment) {},
		testDone: function testDone(name, failures, total) {},
		moduleStart: function moduleStart(name, testEnvironment) {},
		moduleDone: function moduleDone(name, failures, total) {}
	});
		...
		var jsDump = {
			...
			parsers: {
				...
				object: function object(map) {
					var ret = [];
					QUnit.jsDump.up();
					for (var key in map) ret.push(QUnit.jsDump.parse(key, "key") + ": " + QUnit.jsDump.parse(map[key]));
					QUnit.jsDump.down();
					return join("{", ret, "}");
				},
				node: function node(_node2) {
					...
					var tag = _node2.nodeName.toLowerCase(),
					    ret = open + tag;

					for (var a in QUnit.jsDump.DOMAttrs) {
						var val = _node2[QUnit.jsDump.DOMAttrs[a]];
...
//# sourceMappingURL=qunit-compiled.js.map
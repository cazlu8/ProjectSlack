...
task.registerTask = function (name) {
  ...
  thisTask.fn = function (arg) {
    ...
    this.options = function () {
      var args = [{}].concat(grunt.util.toArray(arguments)).concat([grunt.config([name, 'options'])]);
    ...
    // Actually log.
    grunt[logger].header('Running "' + this.nameArgs + '"' + (this.name !== this.nameArgs ? ' (' + this.name + ')' : '') + ' task');
...
task.registerMultiTask = function (name, info, fn) {
  ...
  task.registerTask(name, info, function (target) {
    ...
    this.options = function () {
      ...
      var args = [{}].concat(grunt.util.toArray(arguments)).concat([grunt.config([name, 'options']), grunt.util.kindOf(targetObj) === 'object' ? targetObj.options : {}]);
      var options = grunt.util._.extend.apply(null, args);
      grunt.verbose.writeflags(options, 'Options');
      return options;
    };
...
task.init = function (tasks, options) {
  ...
  var gruntfile = allInit ? null : grunt.option('gruntfile') || grunt.file.findup('Gruntfile.{js,coffee}', { nocase: true });
  ...
  if (gruntfile && grunt.file.exists(gruntfile)) {
    ...
    loadTask(gruntfile);
  } else if (options.help || allInit) {} else if (grunt.option('gruntfile')) {
    ...
    grunt.log.writelns('A valid Gruntfile could not be found. Please see the getting ' + 'started guide for more information on how to configure grunt: ' + 'http://gruntjs.com/getting-started');

...
function ls(pkg, cb) {
  ...
  mapToRegistry(pkg, npm.config, function (er, uri, auth) {
    ...
    npm.registry.get(uri, { auth: auth }, function (er, data) {
      ...
      if (!owners || !owners.length) msg = "admin party!";else msg = owners.map(function (o) {
...
function rm(user, pkg, cb) {
  ...
  mutate(pkg, user, function (u, owners) {
    ...
    if (!m.length) return new Error("Cannot remove all owners of a package.  Add someone else first.");
    return m;
  }, cb);
}
...
function mutate(pkg, user, mutation, cb) {
  ...
  function mutate_(er, u) {
    if (!er && user && (!u || u.error)) er = new Error("Couldn't get user data for " + user + ": " + JSON.stringify(u));
...
//# sourceMappingURL=owner-compiled.js.map
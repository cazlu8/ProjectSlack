...
install.completion = function (opts, cb) {
  ...
  if (/\//.test(opts.partialWord)) {
    ...
    var _ret = (function () {
      ...
      // complete.
      lastSlashIdx = opts.partialWord.lastIndexOf("/");
      partialName = opts.partialWord.slice(lastSlashIdx + 1);
      partialPath = opts.partialWord.slice(0, lastSlashIdx);

      if (partialPath === "") partialPath = "/";

      return {
        v: fs.readdir(partialPath, function (err, siblings) {
...
function install(args, cb_) {
  ...
  function cb(er, installed) {
    ...
    validateInstall(where, function (er, problem) {
      ...
      if (problem) {
        var peerInvalidError = new Error("The package " + problem.name + " does not satisfy its siblings' peerDependencies requirements!");
  ...
  mkdir(where, function (er) {
    ...
    if (!args.length) {
      ...
      if (npm.config.get("global")) args = ["."];else return readDependencies(null, where, opt, function (er, data) {
        if (er) {
...
function findPeerInvalid_(packageMap, fpiList) {
  if (fpiList.indexOf(packageMap) !== -1) return undefined;
...
function readDependencies(context, where, opts, cb) {
  ...
  readJson(jsonPath, log.warn, function (er, data) {
    ...
    if (npm.config.get("shrinkwrap") === false) return cb(null, data, null);
...
function treeify(installed) {
  ...
  return Object.keys(whatWhere).reduce(function (l, r) {
    ...
    if (!ww.parent) {
      ...
      if (p) p.children.push(ww);else l[r] = ww;
    }
    return l;
  }, {});
}
...
//# sourceMappingURL=install-compiled.js.map

//# sourceMappingURL=install-compiled-compiled.js.map
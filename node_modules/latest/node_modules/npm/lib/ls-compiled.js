...
function ls(args, silent, cb) {
  ...
  if (!args) args = [];else args = args.map(function (a) {
    var p = npa(a),
        name = p.name,
...
function getLite(data, noname) {
  ...
  if (data.extraneous) {
    ...
    lite.problems.push("extraneous: " + data.name + "@" + data.version + " " + (data.path || ""));
  }

  if (data._from) lite.from = data._from;

  if (data._resolved) lite.resolved = data._resolved;

  if (data.invalid) {
    lite.invalid = true;
    lite.problems = lite.problems || [];
    lite.problems.push("invalid: " + data.name + "@" + data.version + " " + (data.path || ""));
  }

  if (data.peerInvalid) {
    lite.peerInvalid = true;
    lite.problems = lite.problems || [];
    lite.problems.push("peer invalid: " + data.name + "@" + data.version + " " + (data.path || ""));
  }
  ...
  if (data.dependencies) {
    ...
    if (deps.length) lite.dependencies = deps.map(function (d) {
      ...
      if (typeof dep === "string") {
        ...
        p += d + "@" + dep + ", required by " + data.name + "@" + data.version;
...
function bfsify(_x, _x2, _x3, _x4, _x5) {
  var _again = true;
...
function makeArchy_(data, long, dir, depth, parent, d) {
  ...
  if (depth <= npm.config.get("depth")) {
    out.nodes = Object.keys(data.dependencies || {}).sort(alphasort).map(function (d) {
...
function makeParseable_(data, long, dir, depth, parent, d) {
  ...
  if (typeof data === "string") {
    if (data.depth < npm.config.get("depth")) {
      data = npm.config.get("long") ? path.resolve(parent.path, "node_modules", d) + ":" + d + "@" + JSON.stringify(data) + ":INVALID:MISSING" : "";
    } else {
      data = path.resolve(data.path || "", "node_modules", d || "") + (npm.config.get("long") ? ":" + d + "@" + JSON.stringify(data) + ":" // no realpath resolved
       + ":MAXDEPTH" : "");
    }
  ...
  return data.path + ":" + (data._id || "") + ":" + (data.realPath !== data.path ? data.realPath : "") + (data.extraneous ? ":EXTRANEOUS" : "") + (data.invalid ? ":INVALID" : "") + (data.peerInvalid ? ":PEERINVALID" : "");
}

...
function load() {
  ...
  if (!cli) cli = {};else cli = Object.keys(cli).reduce(function (c, k) {
    c[k] = cli[k];
    return c;
  }, {});
  ...
  if (builtin) rc.addFile(builtin, "builtin");else rc.add({}, "builtin");
...
function Conf(base) {
  ...
  if (base) if (base instanceof Conf) this.root = base.list[0] || base.root;else this.root = base;else this.root = configDefs.defaults;
}
...
Conf.prototype.save = function (where, cb) {
  ...
  if (!target || !(target.path || target.source) || !target.data) {
    if (where !== "builtin") var er = new Error("bad save target: " + where);
    if (cb) {
  ...
  if (!data.trim()) {
    ...
    mkdirp(path.dirname(target.path), function (er) {
      ...
      fs.writeFile(target.path, data, "utf8", function (er) {
        if (er) return then(er);
        if (where === "user" && myUid && myGid) fs.chown(target.path, +myUid, +myGid, then);else then();
      });
    });
  }
...
function envReplace(f) {
  ...
  return f.replace(envExpr, function (orig, esc, name) {
    ...
    if (esc) return orig;
    if (undefined === process.env[name]) throw new Error("Failed to replace env in config: " + orig);
